...
const { visible, listQuery, listLoading, tableData, selectedRow } =
  toRefs(state)
...
const fetchData = () => {
  ...
  getTodayHotVideoRecipe(listQuery.value)
    .then(res => {
...
const onDeleteRecipe = async () => {
  ...
  const { message } = await deleteTodayHotVideoRecipe({ idList })
...
const handleUpdateSortNo = async (row: any) => {
  const { message } = await updateTodayHotVideoRecipeSortNo({
    id: row.id,
    sortNo: row.sortNo
  })
  ElMessage.success(message)
